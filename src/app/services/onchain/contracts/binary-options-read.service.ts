...
export class BinaryOptionsReadService{
 ...
 async getMarket(mk:string):Promise<BinaryMarket|null>{const c=await this.binaryContractOrNull(); if(!c)return null; try{const market=this.mapMarket(await c['getMarket'](mk)); if(market){try{market.openInterest=bi(await c['marketOpenInterest'](mk))}catch{market.openInterest=0n}} return market}catch{return null}}
