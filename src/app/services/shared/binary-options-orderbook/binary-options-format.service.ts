...
export class BinaryOptionsFormatService{
 ...
 formatStrike(v:bigint):string{return `${ethers.formatEther(v)} ETH`}
 ...
 condition(t:number,base:string,strike:bigint):string{const op=t===0?'Above':'Below'; const sign=t===0?'>':'<'; return `${op}: ${base} ${sign} ${this.formatStrike(strike)}`}
}
